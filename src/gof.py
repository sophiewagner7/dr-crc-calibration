...
def objective(log, i):
    ...
    score2 += (
        0.5
        * (1 / np.sqrt(35656))
        * np.square(inc_log[12, :].sum() - c.N * c.polyp_targets[1])
    )  # polyps
    score2 += (
        0.5
        * (1 / np.sqrt(31434))
        * np.square(inc_log[13, :].sum() - c.N * c.polyp_targets[0])
    )  # uCRC

    score = score1 + (score2 * 0.5)

    if i % 5000 == 0:
        print(f"SEER contribution: {score1}, ({round(score1/score*100,0)}%)")
        print(f"Polyp contribution:{score2}, ({round(score2/score*100,0)}%)")
